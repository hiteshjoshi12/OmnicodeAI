...
export default function LanguageDropdown({ options, selected, onChange }) {
              ...
              {options.map((option) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between px-4 py-2.5 text-sm font-bold transition-colors ${
                    selected.id === option.id ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
                  }`}
                >
                  {option.name}
                  {selected.id === option.id && <Check size={16} className="text-indigo-600" />}
                </button>
              ))}
            </div>
          </motion.div>
